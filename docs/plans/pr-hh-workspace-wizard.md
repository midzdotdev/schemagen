...
8. **`onDuplicate` is gone entirely; identity always means "newest per key wins".** First `keep-all` went (functionally redundant with byte-dedup on the no-identity path), then `skip` (its niche cases — audit/lock-in, first-source-authoritative — aren't worth the cognitive overhead in the wizard). `IdentityConfig` is now just `{ fields: Path[] }`. The IdentityPicker drops the on-duplicate radio entirely and the helper text explains the model: "Duplicates on the identity key keep the newest version." A separate note keeps the byte-dedup floor visible: "Either way, schemagen always collapses records that are byte-identical (same fields and values, regardless of key order) — re-imports of the same payload won't pile up." Implemented ahead of the wizard so the dialog already reflects the simplified model.
